...
impl WindowsResource {
    ...
    pub fn write_resource_file<P: AsRef<Path>>(&self, path: P) -> io::Result<()> {
        ...
        self.write_resource(&mut f)
    }

    fn write_resource<W: Write>(&self, f: &mut W) -> io::Result<()> {
...
mod tests {
    ...
    #[test]
    fn resource_serialization_is_deterministic() {
        ...
        for seed in 0..64 {
            let mut fixed_order = [0, 1, 2, 3, 4, 5, 6];
            let mut property_order = [0, 1, 2, 3];
            shuffle(&mut fixed_order, seed);
            shuffle(&mut property_order, seed ^ 0xa5a5_a5a5_a5a5_a5a5);

            let mut version_info = BTreeMap::new();
            for index in fixed_order {
                let (field, value) = match index {
                    0 => (VersionInfo::FILEVERSION, 0x0001_0002_0003_0004),
                    1 => (VersionInfo::PRODUCTVERSION, 0x0005_0006_0007_0008),
                    2 => (VersionInfo::FILEOS, 0x0004_0004),
                    3 => (VersionInfo::FILETYPE, 1),
                    4 => (VersionInfo::FILESUBTYPE, 0),
                    5 => (VersionInfo::FILEFLAGSMASK, 0x3f),
                    _ => (VersionInfo::FILEFLAGS, 0),
                };
                version_info.insert(field, value);
            }
            ...
            for index in property_order {
                property_map.insert(properties[index].0.into(), properties[index].1.into());
            }

            let resource = WindowsResource {
                toolkit_path: PathBuf::new(),
                properties: property_map,
                version_info,
                rc_file: None,
                icons: vec![Icon {
                    path: "LCDSirPlus.ico".into(),
                    name_id: "1".into(),
                }],
                language: 0,
                manifest: None,
                ...
                windres_path: String::new(),
                ar_path: String::new(),
                add_toolkit_include: false,
                append_rc_content: String::new(),
            };
            let mut bytes = Vec::new();
            resource.write_resource(&mut bytes).unwrap();
            if let Some(reference) = expected.as_ref() {
                assert_eq!(&bytes, reference, "insertion seed {seed}");
            } else {
                expected = Some(bytes);
            }
        }
        ...
        let ordered = [
            ...
            "VALUE \"ProductName\", \"Winres\"",
            "VALUE \"ProductVersion\", \"5.6.7.8\"",
            "1 ICON \"LCDSirPlus.ico\"",
        ];
        let mut offset = 0;
        for value in ordered {
            let position = text[offset..].find(value).unwrap();
